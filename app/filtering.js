...
function registerPermissionHandler (session, partition) {
  ...
  session.setPermissionRequestHandler((origin, mainFrameUrl, permissionTypes, muonCb) => {
    ...
    for (let i = 0; i < permissionTypes.length; i++) {
      ...
      const message = locale.translation('permissionMessage').replace(/{{\s*host\s*}}/, parsedMainFrameUrl.hostname || 'Brave Browser').replace(/{{\s*permission\s*}}/, permissions[permission].action)
